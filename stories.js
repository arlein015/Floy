// ========================
// MODULE STORIES - FLOY
// Stories style Instagram (ronds avec dégradé)
// ========================
...
class StoriesModule {
    constructor() {
        ...
        setInterval(() => this.cleanExpiredStories(), 60 * 60 * 1000);
    }
    ...
    openCreateStoryModal() {
        const modalHtml = `
            <div id="storyCreateModal" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:5000;display:flex;align-items:center;justify-content:center;">
                <div style="background:#000;border-radius:16px;width:400px;max-width:90%;border:1px solid #262626;">
                    <div style="padding:16px;border-bottom:1px solid #262626;display:flex;justify-content:space-between;">
                        <h3 style="color:#fff;">Créer une story</h3>
                        <button onclick="this.closest('#storyCreateModal').remove()" style="background:none;border:none;color:#fff;font-size:24px;cursor:pointer;">&times;</button>
                    </div>
                    <div style="padding:20px;">
                        <div class="story-upload-area" onclick="document.getElementById('storyFileInput').click()" style="text-align:center;padding:40px;border:2px dashed #262626;border-radius:12px;cursor:pointer;">
                            ...
                            <p style="color:#8e8e8e;margin-bottom:16px;">Choisissez une photo ou vidéo</p>
                        ...
                        <div id="storyPreview" style="display:none;margin-top:20px;">
                            <img id="storyPreviewImg" style="max-width:100%;max-height:250px;border-radius:12px;margin-bottom:16px;">
                            <video id="storyPreviewVideo" style="max-width:100%;max-height:250px;border-radius:12px;margin-bottom:16px;" controls></video>
                            <button class="story-publish-btn" onclick="storiesModule.publishStory()" style="width:100%;background:#FF6B35;border:none;padding:12px;border-radius:8px;color:#fff;font-weight:600;cursor:pointer;">Publier la story</button>
                        </div>
    ...
    renderStories(containerId = 'stories') {
        ...
        let html = `
            <div class="story-ring" onclick="storiesModule.openCreateStoryModal()">
        ...
        otherUsers.forEach(user => {
            html += `
                <div class="story-ring" onclick="storiesModule.viewUserStories(${user.id})">
    ...
    displayCurrentStory() {
        ...
        const usernameDisplay = document.getElementById('storyViewerUsername');
...
if (!document.getElementById('stories-module-styles')) {
    ...
    styleSheet.id = 'stories-module-styles';
    styleSheet.textContent = storiesStyles;
    document.head.appendChild(styleSheet);
}

// Initialisation
document.addEventListener('DOMContentLoaded', () => {
    console.log("📸 Module Stories chargé");
});
